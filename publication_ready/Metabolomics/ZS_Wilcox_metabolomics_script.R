...
#excluded_metabolites <- read_excel("additional/Metabolomics/filtered for developmental/analiza - wykluczenie metabolitow zwiazanych z rozwojem i terpia VGB.xlsx")
...
#data <- data[,!(colnames(data) %in% excluded_metabolites[[1]])]
  ...
  lapply(function(x) {
    ...
    tryCatch({
      ...
      write.xlsx(results, paste0("publication_ready/Metabolomics/Wilcox_results/", name_comp, ".xlsx"))
...
results_to_save %>% write.xlsx("publication_ready/Metabolomics/Wilcox_results/results.xlsx")

results_to_save %>% 
  count(comparison) %>% 
  write.xlsx("publication_ready/Metabolomics/Wilcox_results/summary.xlsx")
