...
#   left_join(
#     sample_data %>%
#       dplyr::select(samples, 'seizures_any') %>%
...
#   group_by(seizures_any) %>%
#   summarise(n = n()) %>%
#   mutate(thresh = as.integer(n * 0.5)) %>%
#   left_join(
#     DF
#   )
# 
# to_save <-names(DF[,-c(1:3)][apply(DF[,-c(1:3)], 2, count_signif, DF$thresh) > 1])
# 
# data <- data[,to_save] 
...
plot_volcano <- function(results, data, sampling_data, comparison_name) {
  ...
  if (nrow(m1.sig.up) > 0) {
    ...
    #   gather(metabolite, value, -samples) %>%
    ...
    # 
    # ggsave(paste0("~/interim_report/additional/Proteomics/", comparison_name, "_up_reg.png"), width = 9, height = 10, plot = p_up)
  ...
  if (nrow(m1.sig.down) > 0) {
    ...
    # 
    # ggsave(paste0("~/interim_report/additional/Proteomics/", comparison_name, "_down_reg.png"), width = 9, height = 10, plot = p_down)
  ...
  p <- p + theme(legend.title = element_blank()) + 
    scale_color_manual(
      labels = c(
        "1" = paste0(nrow(m1.sig.up), " over-expressed protein groups in ", levels(sampling_data$condition)[1], " (n = ", sum(sampling_data$condition == levels(sampling_data$condition)[1]), ")"),
        "2" = paste0(nrow(m1.sig.down), " over-expressed protein groups in ", levels(sampling_data$condition)[2], " (n = ", sum(sampling_data$condition == levels(sampling_data$condition)[2]), ")")
      ),
      values = c("1" = "red3","2" = "blue3"),
      drop = FALSE
    )
  
  ggsave(paste0("~/interim_report/additional/Proteomics/modification_03_03_age_groups_filter/", comparison_name, ".png"), width = 9, height = 6, plot = p)
}
...
final_results <-
  ...
  lapply(function(x) {
    ...
    DF <-
      ...
      group_by(condition) %>%
      summarise_all(~sum(!is.na(.))) %>% 
      mutate(thresh = as.integer(samples * 0.7))
    
    to_save <-names(DF[,-c(1:3)][apply(DF[,-c(1:3)], 2, count_signif, DF$thresh) > 1])
    
    df <- df[,c("samples", "condition",to_save)] 
    rm(DF)
    tryCatch({
      results <-
        compare_means(df[-1]) 
      ...
      results$group_1_med <- matrixStats::colMedians(group_1, na.rm = T)
      results$group_2_med <- matrixStats::colMedians(group_2, na.rm = T)
      results$log2FoldChange <- matrixStats::colMedians(group_1, na.rm = T) - matrixStats::colMedians(group_2, na.rm = T) 
      results$fold <- 2 ^ results$log2FoldChange
      
      name_comp <- str_replace(x$comparison[1], "/", "") 
      
      write_csv(results, paste0(here::here("publication_ready/Proteomics/Wilcox_results/recalculated_"), name_comp, ".csv"))
...
results_to_save %>% write_csv("~/interim_report/additional/Proteomics/modification_03_03_age_groups_filter/results.csv")

results_to_save %>% 
  count(comparison) %>% 
  write_csv("~/interim_report/additional/Proteomics/modification_03_03_age_groups_filter/summary.csv")
