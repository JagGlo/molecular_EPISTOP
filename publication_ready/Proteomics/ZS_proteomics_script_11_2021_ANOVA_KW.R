...
data <-
  read_excel("publication_ready/data/Proteomics Results for Testing_21_01.xlsx",
...
# Filter data
# (Achromobacter lyticus), (Sus scrofa), (Bos taurus)
# 50% valid values overall
...
comparisons %>%
  ...
  lapply(function(x) {
    ...
    x$condition <- factor(x$condition)
      ...
      write.xlsx(KW_results, paste0("publication_ready/Proteomics/KW_ZScore_cor_", name_comp, ".xlsx"), sheetName= "KW results", append = F)
        
      write.xlsx(results, paste0("publication_ready/Proteomics/KW_ZScore_cor_", name_comp, ".xlsx"), sheetName= "Dunn results", append = T)
...
results_to_save %>% write.xlsx("publication_ready/Proteomics/ZScore_results.xlsx")

results_to_save %>% 
  distinct(comparison, molecule_name) %>% 
  count(comparison) %>% 
  write.xlsx("publication_ready/Proteomics/ZScore_summary.xlsx")
