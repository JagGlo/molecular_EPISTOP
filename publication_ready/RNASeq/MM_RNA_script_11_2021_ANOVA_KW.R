...
convertGeneIDs <- read_csv("additional/RNASeq/convertGeneIDs.csv")

RNA_valid_data <- 
  read_csv(here::here("publication_ready/data/RNA_MM_corrected_data.csv"))
#select(-c(`7042-12-001-031`, `7042-12-001-061`, `7042-12-001-009`, `7042-12-003-188`))
...
final_results <-
  ...
  lapply(function(x) {
    ...
    tryCatch({
      ...
      df <- df[, c("samples", "condition", to_save[[1]])]
      ...
      # write.xlsx(KW_results, paste0("publication_ready/RNASeq/KW_MM_cor_", name_comp, ".xlsx"), sheetName= "KW results", append = F)
      write_csv(KW_results, paste0("publication_ready/RNASeq/KW_results/KW_MM_cor_", name_comp, ".csv"))
      head(results)
      
      # write.xlsx(results, paste0("publication_ready/RNASeq/KW_MM_cor_", name_comp, "_Dunn.xlsx"), append = F)
      write_csv(results, paste0("publication_ready/RNASeq/KW_results/KW_MM_cor_", name_comp, "_Dunn.csv"))
...
results_to_save %>% write_csv("publication_ready/RNASeq/KW_results/MM_results.csv")

results_to_save %>% 
  distinct(comparison, molecule_name) %>% 
  count(comparison) %>% 
  write_csv("publication_ready/RNASeq/KW_results/MM_summary.csv")
