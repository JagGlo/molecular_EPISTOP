...
final_results <-
  ...
  lapply(function(x) {
    ...
    tryCatch({
      ...
      write.xlsx(KW_results, paste0("publication_ready/Metabolomics/KW_results/KW_MM_cor_", name_comp, ".xlsx"), sheetName= "KW results", append = F)
      
      write.xlsx(results, paste0("publication_ready/Metabolomics/KW_results/KW_MM_cor_", name_comp, ".xlsx"), sheetName= "Dunn results", append = T)
...
results_to_save %>% write.xlsx("publication_ready/Metabolomics/KW_results/MM_results.xlsx")

results_to_save %>% 
  distinct(comparison, molecule_name) %>% 
  count(comparison) %>% 
  write.xlsx("publication_ready/Metabolomics/KW_results/MM_summary.xlsx")