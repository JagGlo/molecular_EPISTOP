...
plot_volcano <- function(results, data, sampling_data, comparison_name) {
  ...
  if (nrow(m1.sig) > 0) {
    ...
    if (nrow(m1.sig.up) > 0) {
      ...
      #   data_up <-
      #     data %>% 
      #     select(sample_data, top3_up$feature) %>%
      #     gather(gene, value, -sample_data) %>%
      #     left_join(sampling_data %>% transmute(age_weeks = sample_age_w, sample_data, condition)) %>% 
    ...
    if (nrow(m1.sig.down) > 0) {
      ...
      #   data_down <-
      #     data %>% 
      #     select(sample_data, top3_down$feature) %>%
      #     gather(gene, value, -sample_data) %>%
      #     left_join(sampling_data %>% transmute(age_weeks = sample_age_w, sample_data, condition)) %>% 
      #     left_join(convertGeneIDs %>% select(ensembl_gene_id, hgnc_symbol), by = c("gene" = "ensembl_gene_id")) %>% 
      #     mutate(gene = if_else(!is.na(hgnc_symbol), hgnc_symbol, gene))
...
final_results <-
  ...
  lapply(function(x) {
    ...
    RNA <- RNA_raw[, colnames(RNA_raw) %in% x$sample_data]
    
    annotation <-
      x %>%
      filter(sample_data %in% colnames(RNA)) %>% # not always sample_data are found in data
      dplyr::select(-comparison, -sample_age_w)
    # more than 50% of valid values inside groups
    group_1 <- RNA[,colnames(RNA) %in% annotation$sample_data[annotation$condition == levels(annotation$condition)[1]]]
    group_1 <- group_1[rowSums(group_1 > 1) > ncol(group_1)/2,]
    group_2 <- RNA[,colnames(RNA) %in% annotation$sample_data[annotation$condition == levels(annotation$condition)[2]]]
    ...
    RNA <-
      ...
      as.data.frame() %>% 
      rownames_to_column() %>%
      dplyr::rename(sample_data = rowname) %>% 
      gather(gene_id, value, -sample_data) 
    
    RNA <-
      RNA %>% 
      # anti_join(
      #   sample_data_to_filter %>%
      #     select(gene_id, sample_data)) %>%
      spread(gene_id, value)
    
    df <- 
      annotation %>% 
      select(sample_data, condition) %>% 
      left_join(RNA, by = c("sample_data")) 
  ...
  results_csv %>% 
    ...
    lapply(function(x) {
      ...
      for(i in 1:length(signif_molecules)){
        ...
        `1_before_correction` <- data.frame(`1_before_correction`) %>% rownames_to_column("sample_data")
        `2_after_correction` = unlist(data_after_correction[data_after_correction[,1] == molecule_name,])
        `2_after_correction` <- data.frame(`2_after_correction`) %>% rownames_to_column("sample_data")
        data <- 
          `1_before_correction` %>% 
          left_join(
            `2_after_correction`
          ...
          left_join(
            annotation_set %>% 
              mutate(sample_data = as.character(sample_data)) %>% 
              select(sample_data, sample_age_w, qualified_for_analysis, TSC_control = Condition, comparison = starts_with(comparison_id))
          ) %>% 
          filter(qualified_for_analysis %in% c("yes", "dev_epi")) %>% 
          filter(!is.na(comparison)) %>% 
          distinct(sample_data, .keep_all = T) %>% 
